...
def yahoo():
	try:
		for yahoocom in range(1,1000):
			srachyaaho = urllib.request.urlopen('http://search.yahoo.com/search;_ylt=A0oG7mY5vS9O9h0A1S1XNyoA?p='+dork+'&ei=UTF-8&vm=r&fr=moz35&xargs=0&pstart=1&b='+str(yahoocom)+'1&xa=e3Awd3h7whaNaN2Z9krI0A--,1311837881')
...
def bing():
	try:
		for bingfor in range(1,2000):
			bingurl = urllib.request.urlopen('http://www.bing.com/search?q='+dork+'&go=&filt=all&first='+str(bingfor)+'1&FORM=PERE2')
...
def google():
	try:
		for googlefor1 in range(1,2000):
			amazonurl = urllib.request.urlopen('http://www.google.com/search?hl=en&q='+dork+'&start='+str(googlefor1)+'0')
...
def ask():
		try:
			for ask_for in range(1,1000):
				ask_url = urllib.request.urlopen('http://www.ask.com/web?q='+dork+'&qsrc=1&frstpgo=0&o=0&l=dir&qid=F8FB1F2372E93231E0EDD6BEB4A5B24F&page='+str(ask_for)+'&jss=')
